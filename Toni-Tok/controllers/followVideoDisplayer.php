...
if(isset($_SESSION['user']) && $_SESSION['user'] != null){
    ...
    $currentUserID = $db -> query("SELECT id FROM users WHERE usernames = :currentUser", ['currentUser' => $currentUser]) -> fetchColumn();
    $following = $db -> query("SELECT user_id FROM followed WHERE follower_id = :follower", ['follower' => $currentUserID]) -> fetchColumn();
    $videosID = $db -> query("SELECT video_id FROM createdVideos WHERE user_id = :uploader", ['uploader' => $following]) -> fetchAll(PDO::FETCH_COLUMN);

    foreach ($videosID as $ID) {
        $title = $db -> query("SELECT title FROM videos WHERE id = $ID") -> fetchColumn();
