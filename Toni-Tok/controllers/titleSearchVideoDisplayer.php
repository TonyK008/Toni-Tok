...
if(isset($_SESSION['user']) && $_SESSION['user'] != null){
    ...
    if(isset($_GET['u_id']) && isset($_GET['v_id'])){

        $userID = $_GET['u_id'];

        $videoID = $_GET['v_id'];

        $title = $db -> query("SELECT title FROM videos WHERE id = :video", ['video' => $videoID]) -> fetchColumn();
        ...
        $uploaderID = $db->query("SELECT id FROM users WHERE usernames = :user", ['user' => $uploader])->fetchColumn();
        $link = $db -> query("SELECT link FROM videos WHERE id = :video", ['video' => $videoID]) -> fetchColumn();
        $likes = $db -> query("SELECT likes FROM videos WHERE id = :video", ['video' => $videoID]) -> fetchColumn();
...
<center>
        ...
        ?>
        <p id="videoTitles" style="font-size: 2vw">Uploaded by: <a id="profileLinks" href="<?php if($uploader == $currentUser){echo("/Toni-Tok/controllers/profile.php");}else{echo("/Toni-Tok/HTML/partials/uploaderProfile.php?u_id=$uploaderID");}?>">@<?= $uploader ?></a></p>
